...
exports.convertHubResponse = void 0;
...
function convertHubResponse(request, response = { data: [], meta: {} }, defaultAuthentication) {
    const results = response.data.map(hub_common_1.datasetToContent);
...
exports.convertHubResponse = convertHubResponse;
...
function getResponseMetadata(response) {
    const respQuery = hub_common_1.getProp(response, "meta.queryParameters");
    const query = respQuery ? JSON.stringify(respQuery) : undefined;
    const respAggregations = hub_common_1.getProp(response, "meta.stats.aggs") || {};
    const aggregations = Object.keys(respAggregations).length > 0 ? respAggregations : undefined;
    return {
        count: hub_common_1.getProp(response, "meta.stats.count") || 0,
        total: hub_common_1.getProp(response, "meta.stats.totalCount") || 0,
        hasNext: !!hub_common_1.getProp(response, "meta.next"),
        query,
        aggregations,
    };
}
