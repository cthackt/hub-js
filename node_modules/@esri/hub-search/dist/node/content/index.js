...
exports.searchDatasets = exports.searchContent = exports.ContentSearchService = void 0;
const hub_common_1 = require("@esri/hub-common");
const arcgis_rest_portal_1 = require("@esri/arcgis-rest-portal");
const convert_request_to_portal_params_1 = require("./helpers/convert-request-to-portal-params");
const convert_request_to_hub_params_1 = require("./helpers/convert-request-to-hub-params");
const convert_portal_response_1 = require("./helpers/convert-portal-response");
const convert_hub_response_1 = require("./helpers/convert-hub-response");
/**
...
class ContentSearchService {
     ...
     */
    search(request) {
        const isPortal = hub_common_1.getProp(request, "options.isPortal") || this.isPortal;
        if (isPortal) {
...
async function searchContent(request = { filter: {}, options: {} }) {
    ...
    const response = hub_common_1.getProp(request, "options.isPortal")
    ...
    if (siteModel) {
        ...
        response.results = response.results.map((content) => hub_common_1.setContentSiteUrls(content, siteModel));
    }
    return response;
}
exports.searchContent = searchContent;
function performEnterpriseContentSearch(request, defaultPortal, defaultAuthentication) {
    const requestParams = convert_request_to_portal_params_1.convertToPortalParams(request, defaultPortal, defaultAuthentication);
    return arcgis_rest_portal_1.searchItems(requestParams).then((response) => convert_portal_response_1.convertPortalResponse(requestParams, response));
}
/**
...
function searchDatasets(request) {
    const params = convert_request_to_hub_params_1.convertToHubParams(request);
    const requestOptions = Object.assign(Object.assign({}, getHubRequestOptions(request.options)), { params });
    return hub_common_1.hubApiSearch(requestOptions);
}
exports.searchDatasets = searchDatasets;
function performHubContentSearch(request) {
    const authentication = hub_common_1.getProp(request, "options.authentication");
    return searchDatasets(request).then((response) => {
        const requestParams = convert_request_to_hub_params_1.convertToHubParams(request);
        return convert_hub_response_1.convertHubResponse(requestParams, response, authentication);
    });
}
function getSiteModelFromOptions(options) {
    ...
    return hub_common_1.fetchSite(options.site, ro);
}
// used above
function getHubRequestOptions(options) {
    return {
        ...
        hubApiUrl: hub_common_1.getHubApiUrl(options.portal),
        portal: hub_common_1.getPortalApiUrl(options.portal),
    };
}
