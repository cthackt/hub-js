...
function getContentIdentifier(content, site) {
    ...
    if (_internal_1.isPageType(content.type)) {
...
function getItemHubType(itemOrType) {
    ...
    const itemType = compose_1.normalizeItemType(itemOrType);
    // TODO: not all categories are Hub types, may need to validate
    return collections_1.getCollection(itemType);
}
...
exports.setContentHubId = (content, hubId) => {
 ...
 */
exports.getContentSiteUrls = (content, siteModel) => {
    ...
    const siteUrl = objects_1.getProp(siteModel, "item.url").replace(/\/$/, "");
 ...
 * @returns
 */
exports.setContentSiteUrls = (content, siteModel) => {
    const { relative, absolute: site } = exports.getContentSiteUrls(content, siteModel);
